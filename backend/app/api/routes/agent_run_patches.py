...
from app.models import AgentRun, GeneratedPatch
...
def get_agent_run_patch(run_id: UUID, db: DbSession = None) -> GeneratedPatchResponse:
    ...
    generated_patch = db.scalar(select(GeneratedPatch).where(GeneratedPatch.agent_run_id == run_id))
    if generated_patch is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Generated patch not found.")
